...
// Handle resize size input
bot.on('message', (msg) => {
  const userId = msg.from.id;
  const state = userState[userId];

  if (state?.mode === 'resize_waiting_size' && msg.text) {
    const match = msg.text.match(/^(\d+)[xX×](\d+)$/);
    if (!match) {
      return bot.sendMessage(msg.chat.id, '❌ Invalid format. Send like `800x600`', { parse_mode: 'Markdown' });
    }
    userState[userId] = { mode: 'resize_ready', width: parseInt(match[1]), height: parseInt(match[2]) };
    return bot.sendMessage(msg.chat.id, `✅ Size set to *${match[1]}x${match[2]}*. Now send me the image!`, { parse_mode: 'Markdown' });
  }
